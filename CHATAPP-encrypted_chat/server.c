    ...
    void check_msg(char* buff, int sender_socket,SSL *ssl){
            ...
            if (usertemp != NULL) {
                ...
                for(int i=0; i<=client_count;i++){
                    if(clients[i].socket == sender_socket){
                        memcpy(sender_user, clients[i].name, strlen(clients[i].name));
                        break;
                    }   
                }
                ...
                for (int i = 0; i <= client_count; i++) {
                    ...
                    if (memcmp(clients[i].name,username,strlen(username)) == 0) {
                        char request[strlen("/request ")+strlen(sender_user)+1];
                        sprintf(request, "/request %s", sender_user);
                        send_message(clients[i].ssl,request);
                        break; 
                    }
                }
            ...
            if (usertemp != NULL) {
                ...
                for (int i = 0; i <= client_count; i++) {
                    ...
                    if (memcmp(clients[i].name,username,strlen(username)) == 0) {
                        ...
                        send_message(clients[i].ssl, "Your message request have been accepted.");
                        break; 
                    }
                }
                for(int i=0; i<=client_count;i++){
        ...
        if(strncmp(buff, "/refuse", 7) == 0){
            ...
            if (usertemp != NULL) {
                ...
                for (int i = 0; i <= client_count; i++) {
                    if (memcmp(clients[i].name,username,strlen(username)) == 0) {
                        send_message(clients[i].ssl,"Your message request have been refused.");
                        break; 
                    }
                }
        ...
        if(strncmp(buff, "/online", 7) == 0){
            char *buffer = (char*)malloc(sizeof(char)*(client_count)*21);
    ...
    void *client_handler(void *arg){
        ...
        clients[client_count].request_state = false;
